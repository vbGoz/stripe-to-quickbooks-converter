...
@main
struct StripeToQuickBooksApp: App {
    var body: some Scene {
        WindowGroup {
            ContentView()
        }
...
struct ContentView: View {
        ...
        VStack(spacing: 20) {
                    ...
                    VStack(spacing: 10) {
                        ...
                        if inputFilePath.isEmpty {
                            Text("Click to select your Stripe CSV file")
                                .font(.headline)
                                .foregroundColor(.secondary)
                            Text("Supported formats: .csv files")
                                .font(.caption)
                                .foregroundColor(.secondary)
                        } else {
            ...
            if !inputFilePath.isEmpty {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    Text("📄 QuickBooks CSV: \(generateOutputPreview())")
                        .font(.body)
                        .foregroundColor(.secondary)
                        .padding(.vertical, 8)
                    ...
                    if generateReport {
                        Text("📋 Summary Report: \(generateReportPreview())")
                            .font(.body)
                            .foregroundColor(.secondary)
                            .padding(.vertical, 8)
                            .padding(.horizontal, 12)
    ...
    private func generateOutputPreview() -> String {
        guard !inputFilePath.isEmpty else { return "" }
        return URL(fileURLWithPath: generateDefaultOutputPath()).lastPathComponent
    }
    
    private func generateReportPreview() -> String {
        guard !inputFilePath.isEmpty else { return "" }
        let url = URL(fileURLWithPath: generateDefaultOutputPath())
        return url.deletingPathExtension().lastPathComponent + "_report.txt"
    }
    ...
    private func processFile() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            do {
                ...
                let finalOutputPath = generateDefaultOutputPath()
...
// MARK: - Data Models and Logic (same as before)
